...
const AdminPage: React.FC = () => {
  ...
  useEffect(() => {
    // Check admin
    if (!user?.is_admin) {
        // Give a slight delay to ensure auth load
  ...
  const loadData = async () => {
      ...
      setUsers(u);
      const p = await api.getPosts('all'); // fetch all
      setPosts(p);
  };
  ...
  if (!user || !user.is_admin) return <div className="p-8 text-center">접근 권한 확인 중...</div>;
    ...
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-sm shadow-sm p-6 min-h-screen">
       <div className="flex items-center gap-3 mb-6 border-b border-gray-200 dark:border-gray-700 pb-4">
           ...
           <div>
               ...
               <p className="text-sm text-gray-500">사이트 운영 및 관리 (Firebase Connected)</p>
           </div>
       </div>
                       ...
                       {users.map(u => (
                           <tr key={u.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                               <td className="px-6 py-4 font-bold text-gray-900 dark:text-white">
                                   {u.username} <span className="text-xs text-gray-400 font-normal">({u.email})</span>
                               </td>
